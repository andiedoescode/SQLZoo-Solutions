...
This README file shows the solutions to the [SQLZoo Tutorials](https://www.sqlzoo.net/wiki/SQL_Tutorial). It includes all main sections linked in the index, as well as the quizzes. Images of provided tables have been included where possible.

The repository includes individual SQL files for each of the linked sections as well as a single SQL file with all solutions.

All sections completed 3/8/2024.
...
9. [Self join](#9-self-join)
   ...
   - [9+. COVID 19 Example](#9-covid-19-window-lag)
...
![Table for Self JOIN](images/selfjoin.png)

Edinburgh Buses - This database consists of two tables: stops and routes
stops(id, name): This is a list of areas served by buses. The detail does not really include each actual bus stop - just areas within Edinburgh and whole towns near Edinburgh.
route(num, company, pos, stop): A route is the path through town taken by a bus. As different companies use numbers arbitrarily the num and the company are both required to identify a route.
